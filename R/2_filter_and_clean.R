...
# Remove below detects from data
shuswap_clean <- shuswap_clean %>%
  filter(!RESULT_LETTER %in% c("<") | is.na(RESULT_LETTER))

#Remove duplicate entries (there aren't any for Shuswap), and keep only useful columns
shuswap_clean <- shuswap_clean %>%
  distinct() %>%
  select(-c(BIO_SAMPLE_WEIGHT_TO, TISSUE_TYPE, SAMPLE_SPECIES, SEX, LIFE_STAGE, BIO_SAMPLE_VOLUME, VOLUME_UNIT, BIO_SAMPLE_AREA, AREA_UNIT, BIO_SIZE_FROM, BIO_SIZE_TO, SIZE_UNIT, BIO_SAMPLE_WEIGHT, WEIGHT_UNIT, SIZE_UNIT, BIO_SAMPLE_WEIGHT_FROM, WEIGHT_UNIT_1, SPECIES, RESULT_LIFE_STAGE, DISCHARGE_TO, ANALYZING_AGENCY))

below_detect <- below_detect %>%
  distinct() %>%
  select(-c(BIO_SAMPLE_WEIGHT_TO, TISSUE_TYPE, SAMPLE_SPECIES, SEX, LIFE_STAGE, BIO_SAMPLE_VOLUME, VOLUME_UNIT, BIO_SAMPLE_AREA, AREA_UNIT, BIO_SIZE_FROM, BIO_SIZE_TO, SIZE_UNIT, BIO_SAMPLE_WEIGHT, WEIGHT_UNIT, SIZE_UNIT, BIO_SAMPLE_WEIGHT_FROM, WEIGHT_UNIT_1, SPECIES, RESULT_LIFE_STAGE))

# CREATE CSV OF CLEAN DATA
write.csv(shuswap_clean, 'C:/R Projects/wqo_shuswap/data/shuswap_clean.csv', row.names = FALSE)

# Create CSV of below detect data for later if you want
write.csv(below_detect)
